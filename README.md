...
| **安装与配置**                  | **数据库基础**                | **数据库高级**            | **服务器管理** |
| ------------------------------- | ----------------------------- | ------------------------- | -------------- |
| [安装数据库](#安装数据库)       | [SQL 风格](#sql-风格)         | [事务处理](#事务处理)     |                |
| [命令行工具](#命令行工具)       | [常用数据类型](#常用数据类型) | [显示锁定](#显示锁定)     |                |
| [查看运行参数](#查看运行参数)   | [字段约束](#字段约束)         | [触发器](#触发器)         |                |
| [修改运行参数](#自定义运行参数) | [常用数据函数](#常用数据函数) | [存储过程](#存储过程)     |                |
...
| 分区方式              | 适用场景               | 示例                               |
| --------------------- | ---------------------- | ---------------------------------- |
| **范围分区（RANGE）** | 按时间、数值范围拆分   | 按 `order_date` 按月分区           |
| **列表分区（LIST）**  | 按离散值分类           | 按 `region` 分区（华北/华南/华东） |
| **哈希分区（HASH）**  | 均匀分布数据，避免热点 | 按 `user_id` 哈希到 N 个分区       |
...
* 分区表本身不能有数据
* 分区键必须包含在 `PRIMARY KEY` 中（如果有主键）
* 过多分区（上千个）会增加规划开销，通常建议 < 1000 个
* 旧版本（< 10）没有原生分区，只能用继承 + 触发器模拟
...
| 类型              | 最佳场景                 | 注意事项                                     |
| ----------------- | ------------------------ | -------------------------------------------- |
| **RANGE（范围）** | 时间序列、连续数值       | 分区键最好是连续可排序的字段，如日期、流水号 |
| **LIST（列表）**  | 离散值分类，如地区、部门 | 离散值数量不要太多，否则分区管理复杂         |
| **HASH（哈希）**  | 均匀分布数据，避免热点   | 分区数固定，适合负载均衡但不适合范围查询     |
...
* 选择查询最频繁的列作为分区键
  ...
  * PostgreSQL 的分区裁剪依赖分区键
  * 示例：`order_date` 是时间序列表的分区键
...
* **定期创建/归档分区**
  ...
  * 对时间序列表，定期创建新的未来分区（自动任务或脚本）。
  * 旧数据可以直接 `DROP PARTITION`，性能比 `DELETE` 快很多。

* **避免过多分区**

  * 分区数过多会影响查询规划器性能（一般 < 1000 分区）。

* **使用默认分区（DEFAULT）处理异常值**

  * 对不符合分区规则的数据放入默认分区，防止插入报错。
...
* **监控分区大小**
  ...
  * 使用 `pg_partition_tree` 或 `pg_total_relation_size()` 监控分区大小，避免单个分区过大。
