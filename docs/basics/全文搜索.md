...
- `tsvector` (Text Search Vector)：它是预处理后的文档，它会将一段长文本分词，去除无意义的停用词（如 "the", "a"），并将单词还原为词干
...
1. 分词：将字符串拆分为标记，区分出哪些是单词、数字、HTML 标签或邮件地址
2. 字典处理：标准化把单词统一。例如 "Running" 和 "Ran" 都会变成词根 "run"
3. 停用词过滤：删掉高频但无意义的词（如 "is", "of"）
...
使用 `to_tsvector`（转换文档）和 `to_tsquery`（转换查询条件）进行匹配
