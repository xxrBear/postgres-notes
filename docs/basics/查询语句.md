...
SELECT
    ...
    (
        SELECT COUNT(e.id)
        FROM employee e
        WHERE e.dept_id = d.id -- 再次是一个相关子查询
    ) AS employee_count
FROM
    department d;
```
...
在标准的 SQL JOIN 中，当您使用 JOIN 或 LEFT JOIN 时，右侧的表或子查询是独立于左侧表的，它不能直接引用左侧表中的任何列来进行过滤或计算。LATERAL 关键字打破了这一限制：

- 按行处理： 当外部查询处理左侧表的每一行时，LATERAL 子查询都会针对该行重新执行一次
- 参数化： 左侧行中的值被用作 LATERAL 子查询的输入参数

你可以把 LATERAL 理解为外部表行驱动的 for 循环
