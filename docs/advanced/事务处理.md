...
PostgreSQL 事务处理是指在数据库中执行一系列 SQL 语句，使其成为一个不可分割的操作单元，即要么全部执行成功，要么全部回滚，以确保数据的一致性和完整性。

SQL 标准定义了四种事务隔离级别，分别是未提交读、读已提交、可重复读、串行化。在不同级别上被禁止的现象是：

脏读：事务 A 读取了事务 B 尚未提交的数据。如果事务 B 随后回滚，事务 A 读到的就是从未存在过的虚假数据

不可重复读：事务 A 读取一行记录，事务 B 修改并提交了该行，事务 A 再次读取同一行，发现结果变了

幻读：事务 A 按条件查询出一组记录，事务 B 插入或删除了符合条件的行并提交，事务 A 再次查询，发现多出（或少了）几行

串行化异常：即使避免了上述三种情况，多个事务并发执行的结果与它们一个接一个按顺序执行的结果不一致
...
Postgres 不真正支持读未提交这个级别，而是当作读已提交处理

即，就算你设置了数据库事务级别是读未提交，PG 数据库还是会使用读已提交级别事务隔离
