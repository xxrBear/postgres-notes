...
脏读：一个事务读取了并发的、未提交事务写入的数据

不可重复读：一个事务重新读取之前读取过的数据，发现该数据已被另一个事务修改（该修改在第一次读取后提交）

幻读：一个事务重新执行一个返回满足搜索条件的行的查询，发现满足该条件的行集合因另一个最近提交的事务而发生变化

串行化异常：一组事务成功提交的结果，与按任何可能的顺序逐个运行这些事务的结果不一致
